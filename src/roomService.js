...
// ── Modèle Room ───────────────────────────────────────────────
function createRoom({ hostId, hostName, avatar, theme = 'food', customWords = null, customThemeName = null, maxPlayers = 8 }) {
  ...
  const room = {
    ...
    customThemeName: customThemeName || null,   // lobby | playing | finished
...
function generateCard(theme, customWords) {
  if (customWords && customWords.length >= 24) {
    const pool = shuffle([...customWords.slice(0, 24)]);
